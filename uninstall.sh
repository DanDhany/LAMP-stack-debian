...
# Ini harus dibuat di awal sekali, agar log uninstall bisa ditulis.
...
remove_manual_files() {
    ...
    rm -f /etc/apache2/conf-available/phpmyadmin-handler.conf || true
    rm -f /etc/apache2/conf-available/php-dashboard.conf || true
    ...
    rm -rf /var/www/html/.htaccess || true
    ...
    # --- PENTING: Pindahkan baris penghapusan direktori log ke sini ---
    # Ini harus menjadi salah satu tindakan terakhir yang dilakukan oleh skrip.
    print_info "Menghapus direktori log dashboard (/var/log/dashboard) dan isinya..."
    rm -rf /var/log/dashboard || true # <-- BARIS INI DIPINDAHKAN
}
...
main() {
    ...
    stop_and_disable_services
    purge_packages
    remove_manual_files # Sekarang direktori log dihapus DI DALAM fungsi ini
    final_cleanup_apt
